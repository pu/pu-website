module CacheExpiryHelper
  ...
  def expire_view_cache_for_kid(kid)
    expire_fragment(:key => "kids_row_#{kid.id}")
  ...
  def expire_view_cache_for_parent(parent)
    expire_fragment(:key => "parents_row_#{parent.id}")
  ...
  def expire_view_cache_for_parentship(parentship)
    ...
    expire_fragment(:key => "kids_row_#{parentship.kid.id}")
    expire_fragment(:key => "parents_row_#{parentship.parent.id}")
